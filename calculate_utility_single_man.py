...
def calculate_utility_single_man(h_s_emax, wage_h_part, wage_h_full, husband, t):
    ...
    net_income_single_h_ef    = gross_to_net(husband.kids,    0      ,wage_h_full,year_row)
    net_income_single_h_ep    = gross_to_net(husband.kids,    0      ,wage_h_part,year_row)
    ...
    u_husband_single[2] = float('-inf')    # single husband can't get pregnant
    u_husband_single[4] = float('-inf')    # single husband can't get pregnant
    u_husband_single[6] = float('-inf')    # single husband can't get pregnant
    # husband (potential husband) current utility from each option:
    u_husband_single[1] = (1 / p.alpha0) * pow(budget_c_single_h_ue, p.alpha0) + \
    ...
    if wage_h_full > 0:  # to avoid division by zero
        u_husband_single[3] = (1 / p.alpha0) * pow(budget_c_single_h_ef, p.alpha0) + \
             p.alpha3_h_s * kids_utility_single_h_ef + divorce_cost_h * husband.married
    else:
        u_husband_single[3] = float('-inf')
    if wage_h_part > 0:  # capacity_w=0.5
        u_husband_single[5] = (1 / p.alpha0) * pow(budget_c_single_h_ep, p.alpha0) + \
        ...
        u_husband_single[5] = float('-inf')
    u_husband_single[7] = school_utility_h  # in school-no leisure, no income, but utility from schooling+increase future value
    # calculate expected utility = current utility + emax value if t<T. = current utility + terminal value if t==T

    u_husband = np.empty(7)
    if t == c.max_period:
        u_husband[1]= u_husband_single[1] + p.t1_w*husband.hsg+p.t2_w*husband.sc+p.t3_w*husband.cg+p.t4_w*husband.pc+p.t5_w*husband.exp+p.t13_w*husband.kids
        u_husband[2]= float('-inf') # can't get pregnant at 60
        if wage_h_full > 0:  # to avoid division by zero
            u_husband[3]= u_husband_single[3] + p.t1_w*husband.hsg+p.t2_w*husband.sc+p.t3_w*husband.cg+p.t4_w*husband.pc+p.t5_w*(husband.exp+1)+p.t13_w*husband.kids+p.t16_w #one more year of experience
        else:
            u_husband[3] = float('-inf')
        u_husband[4]= float('-inf') # can't get pregnant at 60
        if wage_h_part > 0:
            u_husband[5]= u_husband_single[5] + p.t1_w*husband.hsg+p.t2_w*husband.sc+p.t3_w*husband.cg+p.t4_w*husband.pc+p.t5_w*(husband.exp+0.5)+p.t13_w*husband.kids+p.t16_w #one more year of experience
        else:
            u_husband[5] = float('-inf')
        u_husband[6] = float('-inf') # can't get pregnant at 60
        u_husband[7]= float('-inf') # can't go to school at 60
    #####################################################################   ADD EMAX    ########################
    # t - time 17-65
    ...
    # if women is pregnant, add 1 to the number of children unless the number is already 4
        ...
        u_husband[1] = u_husband_single[1] + c.beta0 * h_s_emax(t,husband.schooling, husband_exp_index,husband.kids, husband.health, husband_home_time_index,husband.ability_i, husband.mother_educ, husband.mother_marital)
        u_husband[2] = float('-inf') # can't get pregnant after 40
        if wage_h_full > 0:
            husband.exp_index = exp_to_index(husband.exp+1)
            u_husband[3] = u_husband_single[3] + c.beta0 * h_s_emax(t,husband.schooling, husband.exp_index,husband.kids, husband.health, husband.home_time_index,husband.ability_i, husband.mother_educ, husband.mother_marital)
        else:
            u_husband[3] = float('-inf')
        u_husband[4] = float('-inf')
        if wage_h_part > 0:
            husband.exp_index = exp_to_index(husband.exp+0.5)
            u_husband[5] = u_husband_single[5] + c.beta0 * h_s_emax(t,husband.schooling, husband.exp_index,husband.kids, husband.health, husband.home_time_index,husband.ability_i, husband.mother_educ, husband.mother_marital)
        else:
            u_husband[5] = float('-inf')
        u_husband[6] = float('-inf')
        if husband.age < 31:
            school_index = schooly_to_index(husband.years_of_schooling+1)
            u_husband[7] = u_husband_single[7] + c.beta0 * h_s_emax(t,school_index, husband.exp_index,husband.kids, husband.health, husband.home_time_index,husband.ability_i, husband.mother_educ, husband.mother_marital)
        else:
